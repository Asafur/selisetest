...
import { rememberRedirectAfterLogin } from '@/modules/auth/utils/redirect-after-login';
...
export const ClientMiddleware: React.FC<ClientMiddlewareProps> = ({ children }) => {
  ...
  const { isMounted, isAuthenticated } = useAuthState();
  const isPublicRoute = publicRoutes.includes(currentPath) || currentPath.startsWith('/vibe/');
  const requestedPath = `${currentPath}${location.search}`;
  ...
  useLayoutEffect(() => {
    ...
    if (isMounted && !isAuthenticated && !isPublicRoute) {
      rememberRedirectAfterLogin(requestedPath);
      navigate('/login', {
        replace: true,
        state: { from: requestedPath },
      });
    }
  }, [isAuthenticated, isMounted, isPublicRoute, isSSOCallback, navigate, requestedPath]);
